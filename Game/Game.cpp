	...
	try
	{
		...
		for (json object : objects)
		{
			if (object["name"] == "player")
			{
				...
				float x = (m_map->GetPosition().x - m_map->GetWidth() / 2 + object["x"] + m_tileset->GetTileWidth() / 2) * Scale;
				float y = (m_map->GetPosition().y + m_map->GetHeight() / 2 - object["y"] + m_tileset->GetTileHeight() / 2) * Scale;
				auto playerSprite = make_unique<hTTileSprite>(
					m_tileset.get(), object["gid"], DirectX::XMFLOAT3(x, y, 0.0f));
				playerSprite->SetScale(Scale);
				m_player = std::make_unique<Player>(std::move(playerSprite));
